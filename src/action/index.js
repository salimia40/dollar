...
شما به مدیر ارتقا یافتید`,
  ...
  prmMember: async ctx => {
    ...
    ctx.telegram.sendMessage(
      user.userId,
      `همکار گرامی ((${user.name}))
شما به کاربر معمولی تنزل یافتید`,
      Markup.keyboard(btns)
        .resize()
        .extra()
    )
...
شما به کاربر معمولی تنزل یافتید`,
    ...
    ctx.telegram.sendMessage(
      user.userId,
      `همکار گرامی ((${user.name}))
شما به حسابدار ارتقا یافتید از این پس پیغام های حسابداری برای شما ارسال خواهد شد`,
      Markup.keyboard(btns)
        .resize()
        .extra()
    )
