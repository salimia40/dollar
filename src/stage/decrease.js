...
const scene = new Scene('decreaseScene')
scene.enter((ctx) => {
...
scene.hears(/\d+/, async(ctx, next) => {
    ...
    ctx.session.state = 'amountasked'
        ...
        case 'amountasked':
                ...
                var res = await ctx.reply(`do you confirm to discharge ${userId}:${user.name} with ${charge}?`,
                    Markup
                    .inlineKeyboard([
                        [{
                            text: config.btns.confirm,
                            callback_data: 'confirm'
                        }, {
                            text: config.btns.cancel,
