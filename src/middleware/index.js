...
module.exports = {
  ...
  filterMessages: (ctx, next) => {
    ...
    if (helpers.isPrivate(ctx)) {
      ...
      if (helpers.isAdmin(ctx) && /^.../.test(ctx.message.text)) return next()
      if (!Setting.chatEnabled()) ctx.deleteMessage()
      // if(helpers.isAdmin(ctx) && ( ctx.updateType == "message" && ctx.updateSubTypes.includes('text') && ctx.updateSubTypes.length == 1 && /^.../.test(ctx.message.text))) return next()
      if (
        ctx.updateType == 'message' &&
        ctx.updateSubTypes.includes('text') &&
        ctx.updateSubTypes.length == 1
  ...
  checkUserCompleted: (ctx, next) => {
    ...
    ctx.telegram.sendMessage(
      ctx.message.from.id,
      'لطفا ابتدا اقدام به تکمیل حساب خود فرمایید'
    )
  }
}
